...
- 2026-09-02 16dbbd5 dual-pushed (P4, on a77032a): repair/inpaint inserts MiniMaxH3AddGuide + LoadImageMask/SetLatentNoiseMask and lowers denoise; if AddGuide is missing, fail-closed 502/H3RepairUnavailable (no Wan/LTX fallback). 漫剧 pack leans animagineXL40 + IPAdapter 0.85; video engine still H3 FL2VA/Ref2VA. NSFW PIN default still 10Eros; nsfw_variant=dasiwa is opt-in A/B only (:8195 UNETLoader has no such weight, precheck fails if selected). Remix not wired (not on disk / not in registry). :8195 probe 855 nodes: has ImageToVideo/ReferenceToVideo/TurboLoRA/SetLatentNoiseMask/LoadImageMask; no MiniMaxH3AddGuide; SamplerCustomAdvanced has no denoise_mask. Tests: dashbox P0+P4 25 passed; platform P4+P3+P2+routing 67 passed. P0–P4 code knives done. LICENSE/NOTICE/ToIV untouched.
