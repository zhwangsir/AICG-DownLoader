...
- 2026-09-03 151c801 dual-pushed (on 5967131): gateway preview=true OR quality=preview inserts MiniMaxH3TurboLoRA+MiniMaxH3TurboSampler (FL2VA 8 / Ref2VA 4). Final still 20 steps; do not stack content LoRA; SFW turbo is NOT 10Eros. :8790 restarted (设备管家 confirmed). Repair+preview still AddGuide. Files: dashbox/local_gateway/h3_video.py, main.py, tests/test_local_gateway_h3_p3.py. Gateway-side Turbo; does not erase P3 c27f6db workbench Turbo. Device-side H3 untouched. LICENSE/NOTICE/ToIV untouched.

- 2026-09-03 workstation :8195 ComfyUI 0.30.0→0.34.0 (设备管家 + AICG object_info): git a87667f includes #15439. object_info has MiniMaxH3AddGuide; FL2VA/Ref2VA (ImageToVideo/ReferenceToVideo) still present. P4 repair no longer 502 from missing node (fail-closed remains if node absent). SamplerCustomAdvanced still has no denoise_mask input. INT8/Turbo not re-downloaded. Did not SSH spark02. Cluster SoT is ../ToIV/AGENTS.md. LICENSE/NOTICE untouched.

- 2026-09-02 f5a4037 dual-pushed (on 16fb242): drama script/character/storyboard/Context-IR chat LLM and VLM default both http://192.168.71.82:8000/v1, model qwen3.8-flash-next. /gateway/health required llm+vlm = spark01 .82; report no longer shows .84; required_down=[]. start-aigcpannel.py / local_gateway LOCAL_LLM_BASE_URL and LOCAL_CHAT_MODEL same defaults. platform .env.example updated; local gitignored .env updated not committed. Did not SSH spark02; .84 services not stopped. Local :8100/:8790 healthy; :8080 not touched. Tests: platform config/gateway 25 + H3 P2–P6/Context-IR 72; dashbox test_adapter 12. 5 LTX/auto-engine failures are P5/P6 existing pins, not introduced here. Gaps: some comments/RFC/drift scripts still say spark02 .84; dashbox docker settings.db not re-pinned (code CLUSTER_LLM default already spark01). LICENSE/NOTICE/ToIV untouched.

- 2026-09-02 user口径 (via 设备管家): AIGCPannel script/rewrite/vision all use spark01 .82:8000 qwen3.8-flash-next. Do not pull spark02 .84:8000. gateway/health required no longer includes spark02 LLM (llm+vlm both spark01). Services on .84 untouched; spark02 is not whole-cluster retirement (ToIV may still use it). LICENSE/NOTICE/ToIV untouched.

- 2026-09-02 505d039 dual-pushed (on ccfe7a6): drama compose/export default output_fps=24. Canvas compose fixed at 24; EditModal default FPS_OPTIONS[0]=24; FPS_OPTIONS=[24,30,60] (30/60 still optional). schemas Edit/Pipeline output_fps description pinned to 24 (value was already 24). Did not change DashBox episode compose 1080×1920; did not label 1.5× as 2K. Did not SSH spark02. Tests: frontend vitest 92; backend test_p6_compose_export_ltx 11. Still open: voice 3–8s; happyhorse-1.0 still listed as H3 alias; DashBox episode compose still 1080×1920. LICENSE/NOTICE/ToIV untouched.

- 2026-09-02 1d5c2be dual-pushed (P6, on 18f06d8): drama compose/export default 768x1344 (landscape 1344x768). route_video_engine and gateway _select_video_backend no longer auto-LTX; /v1/models lists LTX-2.5 only when LOCAL_LTX_ENABLED=true. LTX code still behind ltx_enabled. H3 final still 768P. DashBox 1.5× not labeled 2K. Did not SSH spark02. Tests: platform 166, dashbox 35, frontend 77. Gaps: voice 3–8s still not done; Canvas/EditModal compose fps still 30 (schema default 24, AICG continuing); happyhorse-1.0 still listed as H3 alias; DashBox episode compose still 1080×1920 (engine export, not drama generate). LICENSE/NOTICE/ToIV untouched.

- 2026-09-02 7623d05 dual-pushed (P5, on 240d34d): drama generate list/UI keeps H3 only (h3-aio/h3-clean); POST rejects wan22-*; illegal action preset maps to h3-aio. Wan JSON still on disk. MiniMax-H3-local template 768P only; UI 768×1344 / 1344×768. One-click orchestrator pins engine=h3, preview=false, quality=final (quality=final overrides global h3_turbo_enabled). VLM default visual_model_name / LOCAL_VLM_MODEL is qwen3.8-flash-next (user: stronger, 1M context, vision); env can still override. Did not SSH spark02. Tests: platform 135; dashbox Wan generate 400 + H3-local 768p; vitest NSFW/VideoModal. Gaps: LTX auto-route code still present but one-click pins H3; compose export labels still 1080x1920; voice 3–8s not done. LICENSE/NOTICE/ToIV untouched.

- 2026-09-02 16dbbd5 dual-pushed (P4, on a77032a): repair/inpaint inserts MiniMaxH3AddGuide + LoadImageMask/SetLatentNoiseMask and lowers BasicScheduler denoise; mask via SetLatentNoiseMask (SamplerCustomAdvanced has no denoise_mask). Missing AddGuide on :8195 fail-closed 502/H3RepairUnavailable (no Wan/LTX fallback). 漫剧 pack leans animagineXL40 keyframes + IPAdapter 0.85; video still H3 FL2VA/Ref2VA. NSFW PIN default still 10Eros; nsfw_variant=dasiwa opt-in A/B only (UNETLoader has minimax_h3_* and 10Eros, no DaSiWa; precheck fails). Remix (civitai 2879272) not wired. P2 last-frame chain still default on; P3 Turbo+content LoRA still rejected. :8195 probe ~1s 855 nodes: MiniMaxH3ImageToVideo/ReferenceToVideo/TurboLoRA/SetLatentNoiseMask/LoadImageMask; no MiniMaxH3AddGuide (needs ComfyUI upgrade, Comfy-Org #15439). Gaps: repair 502 until H3 ComfyUI upgraded; DaSiWa is a hook only; no standalone repair UI (optional fields on VideoGenerateParams). Tests: dashbox P0+P4 25; platform P4+P3+P2+routing 67. P0–P4 code knives done. LICENSE/NOTICE/ToIV untouched.
