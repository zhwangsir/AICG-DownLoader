    ...
    async def _generate_single_image(
        ...
        try:
            ...
            if self.provider == "openrouter":
                ...
                print(f"[NanoBanana Character] 调用 DashBoxAPI ({self.model}) 生成图像...")
                ...
                if not image_bytes and error_detail:
                    print(f"[NanoBanana Character] DashBoxAPI 失败详情: {error_detail}")
                    raise RuntimeError(error_detail)
            else:
                # Google 直连模式
    ...
    async def _generate_with_reference(
        ...
        try:
            ...
            if self.provider == "openrouter":
                ...
                print(f"[NanoBanana Character] 调用 DashBoxAPI ({self.model}) 生成图像（带参考图）...")
                ref_images = []
                if reference_image_bytes:
                    ref_images.append(_named_image_ref(reference_image_bytes, reference_image_name))
                if additional_image_bytes:
