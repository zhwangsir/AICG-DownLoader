...
async def _generate_image(
    *,
    ...
    if generator.provider == "openrouter":
        ...
        if not image_bytes:
            raise ValueError(
                f"DashBoxAPI image generation failed: {error_detail or 'empty image'}"
            )
    else:
...
async def _call_newapi_image_api(
    *,
    ...
    if not api_key:
        return None, "", "DashBoxAPI API key is missing"
    ...
    logger.info("DashBoxAPI image request: %s", request_context)
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
    }
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            logger.info("DashBoxAPI image POST start: %s", request_context.get("endpoint"))
            response = await client.post(
            ...
            logger.info(
                "DashBoxAPI image POST response: status=%s bytes=%s",
            ...
            result = response.json()
            logger.info(
                "DashBoxAPI image POST parsed: data_count=%d keys=%s",
            ...
            if not data:
                ...
                return None, "", f"DashBoxAPI Images response missing data: {sorted(result.keys())}"
            ...
            return None, "", f"DashBoxAPI Images response missing b64_json/url: {first}"
        ...
        logger.warning(
            "DashBoxAPI image failed: status=%s; %s%s; body=%s",
        ...
        logger.warning("DashBoxAPI image request exception: %s", detail)
        return None, "", f"请求异常: {detail}"
    ...
    async def generate_grid(
        ...
        try:
            ...
            if self.provider == "openrouter":
                ...
                print(
                    f"[DashBoxAPI Images] 调用 {self.model} 生成网格图 "
                ...
                if not image_bytes:
                    message = "DashBoxAPI Images 未返回图像数据"
                    if newapi_error:
        ...
        try:
            ...
            if self.provider == "openrouter":
                ...
                if not image_bytes:
                    return GridGenerationResult(
                        success=False,
                        error=f"DashBoxAPI Images 未返回图片: {error_detail or ''}".strip(),
                        generation_time=time.time() - start_time,
                    )
            else:
                from google import genai
    ...
    async def reformat_sketch(
        ...
        try:
            ...
            if self.provider == "openrouter":
                ...
                print(f"[Reformat] 调用 DashBoxAPI Images ({self.model}) 转换 → {target_aspect} ...")
                prompt_text, ref_bytes = self._extract_ref_bytes_from_contents(
                    contents,
                    include_mime=True,
                )
                ...
                if not image_bytes:
                    return GridGenerationResult(
                        success=False,
                        error=(
                            f"[Reformat] DashBoxAPI Images 未返回图像数据: {newapi_error}"
                            if newapi_error
                            else "[Reformat] DashBoxAPI Images 未返回图像数据"
                        ),
                        generation_time=time.time() - start_time,
                    )
            else:
        ...
        try:
            if self.provider == "openrouter":
                ...
                if image_bytes:
                    ...
                    return output_path
                if error_detail:
                    print(f"[DashBoxAPI Render] 失败: {error_detail}")
                return None
            else:
                # ===== Google 直连分支 =====
                from google import genai
...
"""
        ...
        try:
            ...
            if self.provider == "openrouter":
                ...
                if not image_bytes:
                    raise ValueError(
                        f"DashBoxAPI Images 未返回图像数据: {newapi_error}"
                        if newapi_error
                        else "DashBoxAPI Images 未返回图像数据"
                    )
...
"""
        ...
        try:
            ...
            if self.provider == "openrouter":
                ...
                print(f"[StylePreview] 调用 DashBoxAPI Images ({self.model}) 生成预览图...")
                prompt_text, ref_bytes = self._extract_ref_bytes_from_contents(
                    contents,
                    include_mime=True,
                )
                ...
                if not image_bytes and error_detail:
                    print(f"[StylePreview] DashBoxAPI 失败详情: {error_detail}")
            else:
                # ===== Google 直连分支 =====
                from google import genai
                from google.genai import types
