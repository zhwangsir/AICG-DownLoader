...
本服务把两份数据源按 filename 融合为统一的「模型注册表」，标注 downloaded 状态，
供前端模型卡片与工作流/视频 Agent 消费：
...
class ModelRegistryService:
    ...
    def get_registry(self, models_json_path: str | None = None) -> dict[str, Any]:
        ...
        if not isinstance(downloader_records, list):
            downloader_records = []
        # filename → record（models.json 用 filename+subdir 唯一键，这里按 filename 融合）
