...
- P3 Turbo 预览（2026-09-02 `c27f6db`）：`c27f6db` 已双推（P3，叠 `291d994`）：`preview=true` / `quality=preview` 开 Turbo（FL2VA 8 步、Ref2VA 4 步，`MiniMaxH3TurboLoRA`+Sampler）；成片默认 20 步、Turbo 关。SFW turbo LoRA 是 **v4 pruned**，不是 10Eros；NSFW 预览可用 `10Eros_Max_h3_TURBO_ref2va`；Turbo+内容 LoRA 直接拒绝。LICENSE/NOTICE/ToIV 未动。
