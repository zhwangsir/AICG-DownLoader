...
> 最后更新：2026-08-27
> 身份：DashBox 为壳；短剧流水线与下载器是模块。目录 `ALLProject/DashBox`，远程 Gitee/GitHub 均名 DashBox，尖端 `1567fc6`。上游 LICENSE/NOTICE/品牌不要覆盖、不要改成 MIT。
>
...
**2026-08-27 Studio `1567fc6`**：`NSFWDramaStudioNode` 默认 `pipelineEngine=drama`；剧本/首帧 `/api/drama/script|storyboard/generate_async`；失败回退 R18；可切换。TTS/视频/合成仍 R18。web/api 镜像未完整重建（docker cp）。CSP 可能挡 ComfyUI `:8188` 缩略图。
