...
- **🎬 作品页一键成套**：粘贴别人发布的图片/视频作品链接（`civitai.com/images|posts/...`），自动解析页面 Resources used 里的全部模型，勾选后整套下载（精确到作品使用的版本，自动带 SHA256）。注：创作者隐藏生成信息的作品无法解析
...
- **📁 模型库扫描**：遍历 ComfyUI 各 `models/*` 子目录，列出已有模型
- **下载队列**：
